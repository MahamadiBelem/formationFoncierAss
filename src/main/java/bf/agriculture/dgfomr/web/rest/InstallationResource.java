...
public class InstallationResource {
    ...
    public ResponseEntity<List<Installation>> getAllInstallations(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Installations");
        Page<Installation> page;
        if (eagerload) {
            page = installationService.findAllWithEagerRelationships(pageable);
        } else {
            page = installationService.findAll(pageable);
        }
