...
export class ApprenantesComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.apprenantesService
      ...
      .subscribe(
        (res: HttpResponse<IApprenantes[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInApprenantes(): void {
    this.eventSubscriber = this.eventManager.subscribe('apprenantesListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IApprenantes[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/apprenantes'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
