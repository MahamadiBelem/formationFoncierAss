...
export class SortiePromotionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.sortiePromotionService
      ...
      .subscribe(
        (res: HttpResponse<ISortiePromotion[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInSortiePromotions(): void {
    this.eventSubscriber = this.eventManager.subscribe('sortiePromotionListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISortiePromotion[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sortie-promotion'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
