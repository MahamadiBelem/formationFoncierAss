...
export class TypeAmenagementComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.typeAmenagementService
      ...
      .subscribe(
        (res: HttpResponse<ITypeAmenagement[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTypeAmenagements(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeAmenagementListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeAmenagement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-amenagement'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
