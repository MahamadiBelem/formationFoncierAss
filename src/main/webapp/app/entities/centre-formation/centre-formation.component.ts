...
export class CentreFormationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.centreFormationService
      ...
      .subscribe(
        (res: HttpResponse<ICentreFormation[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInCentreFormations();
  }
  ...
  trackId(index: number, item: ICentreFormation): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInCentreFormations(): void {
    this.eventSubscriber = this.eventManager.subscribe('centreFormationListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICentreFormation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/centre-formation'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
