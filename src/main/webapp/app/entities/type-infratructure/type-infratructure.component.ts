...
export class TypeInfratructureComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.typeInfratructureService
      ...
      .subscribe(
        (res: HttpResponse<ITypeInfratructure[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTypeInfratructures(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeInfratructureListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeInfratructure[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-infratructure'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
